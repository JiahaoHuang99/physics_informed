...
from data_pde.dataset_diffusion_reaction_2d_pdebench_d2 import DiffusionReaction2DDataset, DiffusionReaction2DDataseta
...
def train_dr(model,
    ...
    for e in pbar:
        ...
        if f_weight > 0:
            ...
            out = model(a_in)
            ic_loss, f_loss = PINO_DR2D_loss(out, u0, force, particles)
...
def PINO_DR2D_loss(u, u0, force, particles, space_range=1, time_range=5):
...
def subprocess(args):
    ...
    if args.test:
        ...
        a_set = DiffusionReaction2DDataseta(dataset_params=config['data'], split='train')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/fno/FNO-DR2D-PDEBench-debug.yaml', help='Path to the configuration file')
